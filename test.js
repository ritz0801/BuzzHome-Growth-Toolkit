...
let { email, pass, fr, spin, xs, c_user, datr, sb } = require("./credentials.json");
...
(async function automaticComment() {
    try {
        ...
        await tab.manage().addCookie({ name: 'spin', value: spin });
        ...
        try {
            ...
            for (let i = 0; i < article.length; i++) {
                try {
                    ...
                    if ( 0 && isPropertyGroup(foundName) && !isFound && !isInPreviousGroups(foundName)) {
                        ...
                        const filterGroup = ['Apartments for Rent In Ho Chi Minh']
                        ...
                        try {
                            ...
                            // Choose Group Tab
                            await tab.sleep(randomTimeSleep());
                            // driver.findElements(By.xpath("//a[contains(text(),'Groups')]"));
                            ...
                            console.log("found group tab: ", groupTab)
                            ...
                            console.log("Found result group! **")
